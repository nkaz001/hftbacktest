...
class RiskAverseQueueModel:
    ...
    def new(self, order, hbt):
        if order.side == 1:
            order.q[0] = hbt.bid_depth.get(order.price_tick, 0)
        else:
            order.q[0] = hbt.ask_depth.get(order.price_tick, 0)

    def trade(self, order, qty, hbt):
        order.q[0] -= qty

    def depth(self, order, prev_qty, new_qty, hbt):
        order.q[0] = min(order.q[0], new_qty)

    def is_filled(self, order, hbt):
        return round(order.q[0] / hbt.lot_size) < 0
